...
export async function requestServiceQuote(
  ...
  if (contractTitle) {
    messageParts.push(`Related contract: ${contractTitle}.`);
  }
  if (notes) {
    messageParts.push(`Notes: ${notes}`);
  }
  messageParts.push("Please prepare pricing and follow up with renewal/upsell options.");

  await supabase.from("support_requests").insert({
    customer_id: customerId,
    category: "service_quote",
    message: messageParts.join(" "),
    linked_type: linked_id ? "contract" : null,
    linked_id,
    status: "Open",
  });
