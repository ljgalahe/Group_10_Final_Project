...
export function QuotesPendingApprovalSection({
  ...
  quotes: PendingQuote[];
  /** When true, skip outer page chrome (parent already provides a section). */
  embedded?: boolean;
  /** Render only the quote list / empty state (no section heading). */
  listOnly?: boolean;
}) {
  const body =
    quotes.length === 0 ? (
      <p className={`${listOnly ? "mt-0" : "mt-3"} text-sm text-stone-500`}>
        No Quotes Are Waiting For Approval.
      </p>
    ) : (
      <ul
        className={`${listOnly ? "mt-0" : "mt-4"} divide-y overflow-hidden rounded-lg border bg-white ${
          embedded || listOnly
            ? "divide-stone-100 border-stone-200"
            : "divide-amber-100 border-amber-200"
        }`}
      >
        {quotes.map((quote) => {
          const customerName =
    ...
    return (
      <div>
        ...
        <p className="mt-1 text-sm text-stone-500">
          Same queue as Contracts — approve or request changes here.
        </p>
        {body}
      </div>
    );
